...
// ============================================================================
// Image Restoration — single-file interface around the trained restoration
// model (checkpoints/best.pth), served by demo/backend (FastAPI). This is a
// judge-facing demo, separate from the graded KLA submission (eval.py) —
// PRD.md §4 explicitly excludes a web UI from the scored deliverable.
//
...
// shadows, dot-grid texture, pixel display type + monospace body.
// ============================================================================
...
// ---- resampling kernels (real pixel math, used for the bicubic baseline) --
...
async function runBackendRestore({ imageUrl }) {
  ...
  formData.append("file", sourceBlob, "input.png");

  let response;
  try {
    response = await fetch(`${BACKEND_URL}/restore`, { method: "POST", body: formData });
  } catch (e) {
    throw new Error(`Could not reach the restoration backend at ${BACKEND_URL} — is it running? (${e.message})`);
  }
  if (!response.ok) {
  ...
  const outputWidth = Number(response.headers.get("X-Output-Width"));
  const outputHeight = Number(response.headers.get("X-Output-Height"));
  ...
  return {
    restoredUrl, outputWidth, outputHeight, timeMs: tEnd - t0,
    stageTimings: { preprocessMs: tPre - t0, inferenceMs, postprocessMs: tEnd - tPre - inferenceMs },
    isPlaceholder: false, modelName: "checkpoints/best.pth (RestorationNet, 776,705 params)",
  };
}
